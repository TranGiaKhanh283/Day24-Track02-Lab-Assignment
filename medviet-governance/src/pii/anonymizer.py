...
class MedVietAnonymizer:
    ...
    def anonymize_text(self, text: str, strategy: str = "replace") -> str:
        ...
        if strategy == "replace":
            operators = {
                ...
                "VN_CCCD": OperatorConfig("replace", {"new_value": f"{fake.random_number(digits=12, fix_len=True)}"}),
                "VN_PHONE": OperatorConfig("replace", {"new_value": f"0{fake.random_element(elements=('3','5','7','8','9'))}{fake.random_number(digits=8, fix_len=True)}"}),
            }
        elif strategy == "mask":
            operators = {
                ...
                "VN_PHONE": OperatorConfig("mask", {"chars_to_mask": 7, "masking_char": "*", "from_end": True}),
            }
        elif strategy == "hash":
            import hashlib
            def hash_func(text):
    ...
    def anonymize_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Cột số PII - replace trực tiếp để nhanh và đồng nhất
        df_anon["cccd"] = df_anon["cccd"].apply(lambda x: f"{fake.random_number(digits=12, fix_len=True)}")
        df_anon["so_dien_thoai"] = df_anon["so_dien_thoai"].apply(lambda x: f"0{fake.random_element(elements=('3','5','7','8','9'))}{fake.random_number(digits=8, fix_len=True)}")
